...
library(rexpokit)
library(cladoRcpp)     # (if you want to use multicore functionality; some systems/R versions prefer library(parallel), try either)
library(parallel)
library(BioGeoBEARS)
...
#######################################################
wd = np("C:/Users/mauri/Desktop/Novos Testes/Biogeobears")
setwd(wd)
...
pdffn = "Arvore.pdf"
pdf(file=pdffn, width=50, height=1000)

tr = read.tree(trfn)
tr
plot(tr)
title("Restinga Maurício")
...
# PDF plots
#######################################################
pdffn = "Restinga_DEC_vs_DEC+J_M0_unconstrained_v1.pdf"
pdf(pdffn, height=1000, width=50)

#######################################################
# Plot ancestral states - DEC
#######################################################
analysis_titletxt ="Restinga DEC M0_unconstrained"
...
#######################################################
analysis_titletxt ="Restinga DECJ M0_unconstrained"
...
runslow = TRUE
resfn = "Restinga_DIVALIKE_M0_unconstrained_v1.Rdata"
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
resfn = "Restinga_DIVALIKE+J_M0_unconstrained_v1.Rdata"
runslow = TRUE
if (runslow)
{
  #sourceall("/Dropbox/_njm/__packages/BioGeoBEARS_setup/")
...
pdffn = "Restinga_DIVALIKE_vs_DIVALIKE+J_M0_unconstrained_v1.pdf"
pdf(pdffn, height=1000, width=50)

#######################################################
# Plot ancestral states - DIVALIKE
#######################################################
analysis_titletxt ="BioGeoBEARS DIVALIKE on Restinga M0_unconstrained"
...
# Pie chart
plot_BioGeoBEARS_results(results_object, analysis_titletxt, addl_params=list("j"), plotwhat="pie", label.offset=0.2, tipcex=0.3, statecex=0.3, splitcex=0.6, titlecex=0.4, plotsplits=TRUE, cornercoords_loc=scriptdir, include_null_range=TRUE, tr=tr, tipranges=tipranges)

#######################################################
# Plot ancestral states - DIVALIKE+J
#######################################################
analysis_titletxt ="BioGeoBEARS DIVALIKE+J on Restinga M0_unconstrained"
...
# Pie chart
plot_BioGeoBEARS_results(results_object, analysis_titletxt, addl_params=list("j"), plotwhat="pie", label.offset=0.2, tipcex=0.3, statecex=0.3, splitcex=0.3, titlecex=0.4, plotsplits=TRUE, cornercoords_loc=scriptdir, include_null_range=TRUE, tr=tr, tipranges=tipranges)
...
runslow = TRUE
resfn = "Restinga_BAYAREALIKE_M0_unconstrained_v1.Rdata"
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
resfn = "Restinga_BAYAREALIKE+J_M0_unconstrained_v1.Rdata"
runslow = TRUE
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
...
pdffn = "Restinga_BAYAREALIKE_vs_BAYAREALIKE+J_M0_unconstrained_v1.pdf"
pdf(pdffn, height=1000, width=50)

#######################################################
# Plot ancestral states - BAYAREALIKE
#######################################################
analysis_titletxt ="BioGeoBEARS BAYAREALIKE on Restinga M0_unconstrained"
...
#######################################################
analysis_titletxt ="BioGeoBEARS BAYAREALIKE+J on Restinga M0_unconstrained"
...
#######################################################
#non time stratified BSM
#######################################################
wd = np("C:/Users/mauri/Desktop/Novos Testes/BSM")
setwd(wd)
...
pdffn = paste0("Restinga_BSM", model_name, "_v1.pdf")
pdf(pdffn, height=1000, width=50)

analysis_titletxt = paste0(model_name, " on Restinga")
...
pdf(file=pdffn, height=1000, width=50)

# Convert the BSM into a modified res object
master_table_cladogenetic_events = clado_events_tables[[1]]
resmod = stochastic_map_states_into_res(res=res, master_table_cladogenetic_events=master_table_cladogenetic_events, stratified=FALSE)
...
paint_stochastic_map_branches(res=resmod, master_table_cladogenetic_events=master_table_cladogenetic_events, colors_list_for_states=colors_list_for_states, lwd=5, lty=par("lty"), root.edge=TRUE, stratified=FALSE)
